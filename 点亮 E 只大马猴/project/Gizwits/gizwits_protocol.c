...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagMottor)
    {
        bitFieldBitLen += Mottor_LEN;
    }
    if(0x01 == aFlag->flaghumier)
    {
        bitFieldBitLen += humier_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagMottor)
    {
        info->event[info->num] = EVENT_Mottor;
        info->num++;
        dataPoints->valueMottor = gizVarlenDecompressionValue(bitFieldOffset,Mottor_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += Mottor_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flaghumier)
    {
        info->event[info->num] = EVENT_humier;
        info->num++;
        dataPoints->valuehumier = gizVarlenDecompressionValue(bitFieldOffset,humier_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += humier_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueMottor != cur->valueMottor)
    {
        GIZWITS_LOG("valueMottor Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagMottor = 1;
        ret = 1;
    }
    if(last->valuehumier != cur->valuehumier)
    {
        GIZWITS_LOG("valuehumier Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flaghumier = 1;
        ret = 1;
    }
    ...
    if(last->valueair_quality != cur->valueair_quality)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueair_quality Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagair_quality = 1;
            ret = 1;
        }
    }
    if(last->valueLight_intensity != cur->valueLight_intensity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagLight_intensity = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagMottor)
    {
        gizVarlenCompressValue(bitFieldOffset,Mottor_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueMottor);
        bitFieldOffset += Mottor_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaghumier)
    {
        gizVarlenCompressValue(bitFieldOffset,humier_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuehumier);
        bitFieldOffset += humier_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagair_quality)
    {
        devStatusTmp.valueair_quality = gizY2X(air_quality_RATIO,  air_quality_ADDITION, dataPoints->valueair_quality); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueair_quality,air_quality_LEN);
        byteFieldOffset += air_quality_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagLight_intensity)
    {
        devStatusTmp.valueLight_intensity = gizY2X(Light_intensity_RATIO,  Light_intensity_ADDITION, dataPoints->valueLight_intensity); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueLight_intensity,Light_intensity_LEN);
        byteFieldOffset += Light_intensity_LEN;
    }
